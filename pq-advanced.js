...
// Clicking on the chest image opens the chest to reveal a potion
function openChest() {
...
function drawStatMod(statName, statMod) {
    ...
    let spanId = "#" + statName + "Change";

    // Modification amount to be written to the screen
    let changeAmountText;

    // Prepend a "+" in front of positive numbers
    if (statMod > 0) {
        changeAmountText = "+" + statMod;
    } else {
        // Negatives will already have a "-" sign
        changeAmountText = statMod;
    }
...
function drawHero() {
    ...
    let heroPotions = hero.getPotions();

    // Loop through the hero's backpack array
    for (let index = 0; index < heroPotions.length; index++) {
